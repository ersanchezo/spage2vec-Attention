...
df = pd.read_csv('data/pseudo_ct2.csv')
# read data
...
spots = df[['spotX', 'spotY']].to_numpy()
df_edges = pd.DataFrame(data = knn_graph(spots),columns = ['source', 'target'])
...
df_nodes = df['gene']
df_nodes.to_csv(os.path.join(result_dir, 'knn_nodes_psuedo2.csv'))
#df_edges = nx.to_pandas_edgelist(g)
#df_edges = df_edges.set_index('index')
df_edges.to_csv(os.path.join(result_dir, 'knn_edges_pseudo2.csv'), index=False)
